...
if Config.ESX then
    ...
    AddEventHandler('esx:playerLoaded', function(xPlayer)
        ...
        if xPlayer.job2 then
            CurrenPlayerData2.job = xPlayer.job2 
        end
    end)

    RegisterNetEvent('esx:setJob')
    AddEventHandler('esx:setJob', function(job)
        CurrenPlayerData.job = job
    end)

    RegisterNetEvent('esx:setJob2')
    AddEventHandler('esx:setJob2', function(job)
        CurrenPlayerData2.job = job
    end)
...
elseif Config.QbCore then
    ...
    RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
        CurrenPlayerData.job = FrameWork.Functions.GetPlayerData().job
    end)
    
    RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)
        CurrenPlayerData.job = JobInfo
    end)
end
...
local function HasJob(data)
    ...
    if type(data.job) == "table" then
        for i = 1, #data.job, 1 do
            if 
            data.job[i] == CurrenPlayerData.job.name and CurrenPlayerData.job.grade >= data.grade
            or 
            data.job[i] == CurrenPlayerData2.job.name and CurrenPlayerData2.job.grade >= data.grade
            then
                hasJob = true
            end
        end
    elseif type(data.job) == "string" then 
        if
        data.job == CurrenPlayerData.job.name and CurrenPlayerData.job.grade >= data.grade
        or
        data.job == CurrenPlayerData2.job.name and CurrenPlayerData2.job.grade >= data.grade
        then
            hasJob = true
        end
    end
...
local registerMarker = function (data)
    ...
    local point = lib.points.new({
        ...
        textureDict = data.textureDict or "marker",
    ...
    function point:nearby()
        ...
        if self.show3D and self.msg ~= "" then
            DrawText3D(self.coords.x, self.coords.y, self.coords.z, self.msg)
        end
        ...
        elseif self.texture ~= nil then 
            if not HasStreamedTextureDictLoaded(self.textureDict) then
                ...
                DrawMarker(9, self.coords, 0.0, 0.0, 0.0, 90.0, 0.0, 0.0, self.scale.x, self.scale.y, self.scale.z, 120, 170, 130, 100, false, true, 2, true, self.textureDict, self.texture, false)
            end       
        end
        if self.currentDistance <= self.interactDistance and IsControlJustReleased(0, (type(self.control) == "number") and self.control or (Keys[self.control]) and Keys[self.control] or 38) then
...
local unregisterMarker = function (name)
    if zoneRegistered[name] then
        if zoneRegistered[name].remove and type(zoneRegistered[name].remove) == "function" then
            local status, err = pcall(zoneRegistered[name].remove)
        end
